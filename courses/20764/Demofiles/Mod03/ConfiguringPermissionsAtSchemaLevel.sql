--Module 03 - Configuring Permissions at the Schema Level
...
-- Step 2: Revoke permission to execute dbo.uspGetManagerEmployees from
--         Mod03Login.

REVOKE EXECUTE ON dbo.uspGetManagerEmployees FROM Mod03Login;
GO
...
GRANT EXECUTE ON SCHEMA::dbo TO Mod03Login;
GO
...
-- Question: The user has EXECUTE at the schema level
--           and DENY at the proc level. Should execution
--           be permitted?
...
-- Step 10: Explore which permissions imply the ability
--          to SELECT from a schema.

SELECT * FROM dbo.ImplyingPermissions('schema', 'select');
GO

-- Step 11: Explore which permissions imply the ability
--          to view the definition of an object.

SELECT * FROM dbo.ImplyingPermissions('object', 'view definition');
GO

-- Step 12: Explore which permissions imply the ability
