...
(
    SELECT
        ...
        ROW_NUMBER() OVER
        (
            PARTITION BY E.[InstanceId], E.[JobId]
            ORDER BY E.[RunAt] DESC, E.[JobExecutionId] DESC
        ) AS rn
...
(
    SELECT
        H.[InstanceId],
        H.[DatabaseName],
        MAX(CASE WHEN H.[BackupType] = 'D' AND H.[IsCopyOnly] = 0 THEN H.[BackupFinishDate] END) AS [LastFull],
        MAX(CASE WHEN H.[BackupType] = 'I' THEN H.[BackupFinishDate] END) AS [LastDiff],
        MAX(CASE WHEN H.[BackupType] = 'L' THEN H.[BackupFinishDate] END) AS [LastLog]
    FROM [backup].[BackupHistory] AS H
    GROUP BY H.[InstanceId], H.[DatabaseName]
)
SELECT
    D.[InstanceId],
    D.[ServerInstance],
    ...
    B.[LastDiff],
    B.[LastLog],
    DATEDIFF(hour, B.[LastFull], SYSDATETIME()) AS [FullAgeHours],
    DATEDIFF(minute, B.[LastLog], SYSDATETIME()) AS [LogAgeMinutes],
    CASE
        WHEN B.[LastFull] IS NULL THEN 'NO_FULL'
        WHEN D.[RecoveryModelDesc] = 'FULL' AND B.[LastLog] IS NULL THEN 'NO_LOG'
        WHEN D.[RecoveryModelDesc] = 'FULL' AND DATEDIFF(minute, B.[LastLog], SYSDATETIME()) > 60 THEN 'OLD_LOG'
        WHEN DATEDIFF(hour, B.[LastFull], SYSDATETIME()) > 36 THEN 'OLD_FULL'
        ELSE 'OK'
    END AS [BackupStatus]
FROM [report].[vCurrentDatabases] AS D
LEFT JOIN B
...
(
    SELECT
        ...
        (
            ...
            ORDER BY P.[AssessedAt] DESC, P.[PatchAssessmentId] DESC
        ) AS rn
...
WHERE B.[CollectionStatus] IN ('SUCCESS','PARTIAL');
GO

CREATE OR ALTER VIEW [report].[vGrafanaFileIoTimeSeries]
AS
SELECT
    F.[CapturedAt] AS [Time],
    F.[InstanceId],
    I.[ServerInstance],
    E.[EnvironmentCode],
    F.[DatabaseName],
