...
function New-SqlConnection {
    ...
    $b.ApplicationName='DBACentralRepository v3 Collector'
    $b.ConnectTimeout=15
    $b.Encrypt=$false
    $b.TrustServerCertificate=$true
    if($null -eq $Credential){$b.IntegratedSecurity=$true}
    else{
        $b.UserID=$Credential.UserName
...
function Invoke-Table {
    ...
    $cn=New-SqlConnection $Server $Database $Credential
    try{
        $cn.Open()
